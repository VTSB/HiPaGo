...
export async function getSyncStatus(tag: string): Promise<string | null> {
  ...
  const rows = await db.query<{ data: string }>('SELECT data FROM sync_status WHERE tag = ?', [
    tag,
  ]);
...
export async function setSyncStatus(tag: string, data: string): Promise<void> {
  const db = await ensureDb();
  await db.execute('INSERT OR REPLACE INTO sync_status (tag, data) VALUES (?, ?)', [tag, data]);
}
