...
export async function saveGalleryBlock(block: GalleryBlock): Promise<void> {
  await withTransaction(async (transactionDb) => {
    ...
    await saveGalleryRelated(block.id, block.related, transactionDb);
    await saveGalleryTags(block.id, block.tags, transactionDb);
  });
...
export async function getGalleryBlock(id: number): Promise<GalleryBlock | null> {
  ...
  const rows = await db.query<{
    ...
    'SELECT id, type, title, date, thumbnail, language, mediaType, updatedAt FROM gallery WHERE id = ? AND type > 0',
    [id],
  );
  ...
  return {
    ...
    type: Object.values(GalleryBlockType).includes(row.type)
      ? (row.type as GalleryBlockType)
      : GalleryBlockType.FAILED,
...
export async function getRecentlyViewedWithDates(
  limit = 500,
): Promise<Array<{ galleryId: number; viewedAt: string }>> {
  const db = await ensureDb();
...
export async function saveGalleryImages(galleryId: number, files: GalleryFile[]): Promise<void> {
  await withTransaction(async (transactionDb) => {
    await transactionDb.execute('DELETE FROM gallery_image WHERE galleryId = ?', [galleryId]);
...
export async function cleanupStaleCache(days = 30): Promise<number> {
  ...
  return withTransaction(async (transactionDb) => {
    ...
    await transactionDb.execute('DELETE FROM gallery WHERE type = -1');
    // Clean orphaned junction rows
    await transactionDb.execute('DELETE FROM gallery_tag WHERE id NOT IN (SELECT id FROM gallery)');
    await transactionDb.execute(
      'DELETE FROM gallery_relate WHERE id NOT IN (SELECT id FROM gallery)',
    );

    const cutoff = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();

    // Get stale gallery IDs (exclude favorites)
    const stale = await transactionDb.query<{ id: number }>(
    ...
    for (let i = 0; i < ids.length; i += CHUNK) {
      ...
      const placeholders = chunk.map(() => '?').join(',');
      await transactionDb.execute(
        `DELETE FROM gallery_image WHERE galleryId IN (${placeholders})`,
        chunk,
      );
      await transactionDb.execute(`DELETE FROM gallery_tag WHERE id IN (${placeholders})`, chunk);
      await transactionDb.execute(
        `DELETE FROM gallery_relate WHERE id IN (${placeholders})`,
        chunk,
      );
      await transactionDb.execute(`DELETE FROM gallery WHERE id IN (${placeholders})`, chunk);
    }
