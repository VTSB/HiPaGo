...
describe('listScrollSnapshot', () => {
  ...
  it('falls back to the sessionStorage mirror when history.state is wiped', () => {
    // Simulate what router.replace does on mobile back navigation: the
    // history entry that used to hold the snapshot has been replaced and
    // its state no longer carries hipagoListScrollSnapshot.
    window.history.replaceState({ unrelatedRouterState: true }, '', '/?at=25');
    sessionStorage.setItem(
      'hipago:list-scroll-snapshot-v2',
      JSON.stringify({
        version: 1,
        url: '/?at=25',
        at: 25,
  ...
  it('captureListScrollSnapshot mirrors the snapshot into sessionStorage', () => {
    makeItem(24, -210, -20);
    makeItem(25, 80, 350);
    const clicked = makeItem(26, 360, 630);
    ...
    const raw = sessionStorage.getItem('hipago:list-scroll-snapshot-v2');
